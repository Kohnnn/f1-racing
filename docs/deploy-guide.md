...
- **Frontend**: Cloudflare Pages (recommended), Netlify, or Vercel
...
└────────────────┬────────────────────────────────────────────┘
     ...
     ▼           ▼           ▼
┌─────────┐ ┌─────────┐ ┌─────────┐ ┌──────────┐
│ Cloudflare│ │ Netlify │ │ Vercel  │ │ OCI VM   │  ← pick one for frontend
│ Pages    │ │ (backup)│ │ (backup)│ │ (backend)│
└────┬────┘ └─────────┘ └─────────┘ └────┬─────┘
     │                        │
     │                        ▼
     │                 ┌────────────┐
...
Replay workspace behavior:
- Track drawing, marker hit-testing, and display-order correction use a dense client-side track projection layer.
- Playback supports `0.1x`, `0.2x`, `0.5x`, `1x`, `2x`, `4x`, `8x`, `16x`, and `20x`.
- Skip controls support `5s`, `30s`, `1m`, and `5m`.
- Hotkeys: `Space`, arrow keys, `Shift + Arrow`, `[` / `]`, `1` through `5`, `R`, `D`, `L`, and `B`.
- 2026 Japan Grand Prix Race is generated by the current pipeline and appears in the replay library. Add more 2026 sessions only after their packs exist in both canonical and public data trees.
...
The app is a static Next export. Vercel should deploy the generated `apps/web/out` output, not a serverful `.next` runtime.
...
Manual preview deploy from repo root:

```powershell
npm run build
npx vercel deploy --prebuilt -y
```

If deploying without a prebuild, use:

```powershell
npx vercel deploy -y
```

Production deploy, only when intentionally publishing production:

```powershell
npx vercel deploy --prod --prebuilt -y
```

Vercel environment variable to set in the dashboard if overriding `vercel.json`:

```text
NEXT_PUBLIC_F1_API_ORIGIN=https://f1-api.129.150.58.64.sslip.io
```

Do not add private `.env` values such as Appwrite API keys, Netlify tokens, or OCI SSH commands to public frontend variables.
